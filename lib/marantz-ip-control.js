...
module.exports = class MarantzIPController extends EventEmitter {
  ...
  request(prop, callback) {
    var cmd = buildAndCheckCommand("REQ", prop)
    return writeAndWait(this, cmd, true, callback)
  }
  ...
  act(prop, val, callback) {
    var cmd = buildAndCheckCommand("ACT", prop)
    if (typeof val == 'function') callback = val
    else if (val) cmd = cmd.replace("%s", val)
    return writeAndWait(this, cmd, false, callback)
  }
...
function buildAndCheckCommand(type, prop) {
  var cmd = type.toUpperCase() + "_" + prop.replace(/([A-Z])/g, "_$1").toUpperCase()
  if (!Command[cmd]) throw new Error("Unknown command " + cmd)
  return Command[cmd]
}
 ...
 */
function writeAndWait(controller, command, expectResponse, callback) {
